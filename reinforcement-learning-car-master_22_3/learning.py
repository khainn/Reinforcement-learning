...
def train_net(model, params):
    ...
    while t < train_frames:
        ...
        if reward == -500:
            ...
            start_time = timeit.default_timer()
        if t % 10 == 0:
            print("Max_car_distance: %d at %d\tepsilon %f\t(%d)\tdistance %d" %
                 (max_car_distance, t, epsilon, car_distance, distance ))
...
def process_minibatch2(minibatch, model):
    ...
    mb_len = len(minibatch)
                                               # []: liss, (): tuble 
    old_states = np.zeros(shape=(mb_len, 6))   #tao ra 1 tuble gom 64 liss, moi liss gom 3 phan tu 0
